...
export async function devLogin() {
  ...
  if (!(await prisma.allowedEmailDomain.findUnique({ where: { domain: "kmutt.ac.th" } }))?.isActive) redirect("/unauthorized");
  ...
  if (user.accountStatus !== "ACTIVE" || user.deletedAt) redirect("/unauthorized");
